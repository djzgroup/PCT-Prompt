...
from modules.recons_utils import cal_const, cal_normal, cal_center, check_nan_umb,check_nan_feature_2d,check_nan_feature_1d
...
def sample_and_group(npoint, radius, nsample, center, normal, feature, return_normal=True, return_polar=False, cuda=False):
    ...
    # group
    idx = query_ball_point(radius, nsample, center, new_center)
    torch.cuda.empty_cache()
    # group normal
    group_normal = index_points(normal, idx)  # [B, npoint, nsample, B]
    torch.cuda.empty_cache()
    # group center
    group_center = index_points(center, idx)  # [B, npoint, nsample, A]
    torch.cuda.empty_cache()
    ...
    if feature is not None:
        group_feature = index_points(feature, idx)
        group_center_norm, group_normal, group_feature=check_nan_umb(group_center_norm, group_normal, group_feature)
...
class SurfaceAbstractionCD(nn.Module):
    ...
    def forward(self, center, normal, feature):
        ...
        # group_normal, group_center = check_nan_umb(new_normal, new_center)
        new_feature = new_feature.permute(0, 3, 2, 1)

        # init layer
        # loc = self.bn_l0(self.mlp_l0(new_feature[:, :self.pos_channel]))
        # feat = self.bn_f0(self.mlp_f0(new_feature[:, self.pos_channel:]))
        feat = self.mlp_f0(new_feature[:, self.pos_channel:])
        feat=check_nan_feature_2d(feat.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)

        new_feature =feat
        new_feature = F.leaky_relu(new_feature,0.1)
        new_feature= check_nan_feature_2d(new_feature.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)

        for i, conv in enumerate(self.mlp_convs):
            bn = self.mlp_bns[i]
            new_feature = F.leaky_relu(conv(new_feature),0.1)
            new_feature= check_nan_feature_2d(new_feature.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)
        new_feature = torch.max(new_feature, 2)[0]
...
class UmbrellaSurfaceConstructor(nn.Module):
    ...
    def forward(self, center):
        ...
        # group_xyz =check_nan_feature_2d(group_xyz)
        # normal
        group_normal = cal_normal(group_xyz, random_inv=self.random_inv, is_group=True)
        # coordinate
        group_normal=check_nan_feature_2d(group_normal)
        group_center = cal_center(group_xyz)
        # polar
        group_center=check_nan_feature_2d(group_center)
        group_polar = xyz2sphere(group_center)
        if self.return_dist:
